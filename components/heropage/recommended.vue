   ...
   <div>
      ...
      <div class="flex flex-col gap-4 mt-4">
         <NuxtLink :to="anime.endpoint" class="flex gap-4 p-4 shadow-card hover:shadow-xl hover:shadow-black/10 transition duration-300 rounded-xl bg-white items-center" v-for="manga in anime.data" :key="anime.id">
            ...
            <div>
               <h5 class="font-semibold line-clamp-1">{{ anime.title }}</h5>
               <h6 class="text-sm text-gray-500">{{ anime.type }}</h6>
               <p class="text-[10px] bg-blue-100 text-blue-500 px-2 py-1 inline-flex rounded-full mt-2">{{ anime.desc.slice(0, 18) }}</p>
            </div>
         </NuxtLink>
      </div>
   </div>
